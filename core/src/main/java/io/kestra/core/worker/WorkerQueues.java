 ...
 *
 * <p>The Worker Queue concept lives in the EE module ({@code io.kestra.ee.models.server.WorkerQueue}),
 * but the OSS executor needs to inspect a queue uid to decide whether it refers to the
 * default queue and to render readable log messages. These helpers operate on the uid
 * (and optionally the {@code using} tags the user typed in their flow) without depending
 * on the EE entity.
 */
