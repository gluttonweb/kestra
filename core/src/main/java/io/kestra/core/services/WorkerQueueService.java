 ...
 *
 * <p>Worker Queue routing is an Enterprise Edition feature. OSS supplies the
 * {@link Default} implementation, which only enforces the {@link SystemTask} reservation
 * to the in-process SystemWorker and otherwise returns no routing (jobs run on the
 * global default queue). EE extends {@link Default} and overrides
 * {@link Default#doResolveWorkerQueueForJob(FlowInterface, WorkerJob)} to add tag-based
 * resolution.
 */
public interface WorkerQueueService {
    ...
    /**
     * OSS implementation of {@link WorkerQueueService}.
     *
     ...
     * which on OSS returns no routing. EE overrides this hook to add tag-based resolution.
     */
    @Slf4j
    @Singleton
    ...
    class Default implements WorkerQueueService {
         ...
         * The OSS implementation returns {@link Optional#empty()} (no routing → default
         * queue). EE overrides this hook to add tag-based resolution.
         *
