...
<script setup lang="ts">
    ...
    const props = withDefaults(defineProps<{
        ...
        pebble: undefined,
        duplicateTaskIdMarkers: undefined,
    ...
    const options = computed<EditorOptions>(() => {
        ...
        if (props.inline && !props.lineNumbers) {
        ...
        opts.minimap = props.minimap ? undefined : {enabled: false}
    ...
    function highlightInitial() {
        if (!props.initialHighlight) return
        ...
        for (let i = 0; i < lines.length; i++) {
            if (lines[i].includes(props.initialHighlight)) {
                lineNumber = i + 1
                break
            }
        }
    ...
    function loadScrollData<T>(key: string, fallback?: T): T | undefined {
        if (!props.scrollKey) return fallback
        try {
            const raw = localStorage.getItem(`editorScroll:${props.scrollKey}:${key}`)
    ...
    function saveScrollData<T>(data: T, key: string) {
        ...
        try {
            localStorage.setItem(`editorScroll:${props.scrollKey}:${key}`, JSON.stringify(data))
        } catch {
            // ignore
        }
    }
    ...
    async function initMonaco() {
        const editorOpts: EditorOptions = {
            ...
            ...(props.inFlowEditor ? {padding: {top: 16}} : {}),
            ...options.value,
        }
    ...
    function editorDidMount(monacoMounted?: monaco.editor.IStandaloneCodeEditor | monaco.editor.IStandaloneDiffEditor) {
        ...
        ed.onDidFocusEditorText?.(() => {
            TabFocus.setTabFocusMode(props.keepFocused === undefined ? props.inline : false)
        })
        ...
        if (!props.fullHeight) {
            ed.onDidContentSizeChange((e2) => {
                ...
                container.value.style.height = e2.contentHeight + props.customHeight + "px"
            })
        }
        ...
        if (!isDiff.value) {
            ...
            ed.onDidChangeCursorPosition?.(() => {
                ...
                lastTimeout = window.setTimeout(() => {
                    ...
                    if (props.scrollKey) saveScrollData(codeEditor.saveViewState(), "viewState")
                }, 100)
                highlightPebble()
            })
        }
